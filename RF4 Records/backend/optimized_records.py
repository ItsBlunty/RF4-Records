...
def get_filter_values_optimized():
    ...
    try:
        ...
        fish_query = db.query(distinct(Record.fish)).filter(Record.fish.isnot(None), Record.fish != '')
        waterbody_query = db.query(distinct(Record.waterbody)).filter(Record.waterbody.isnot(None), Record.waterbody != '')
        
        # For bait, we need to handle the composite display format
        # Use raw SQL for better performance with complex bait logic
        bait_query = db.execute(text("""
            SELECT DISTINCT 
                CASE 
                    WHEN bait2 IS NOT NULL AND bait2 != '' 
                    THEN bait1 || '; ' || bait2
                    ELSE COALESCE(bait1, bait, '')
                END as bait_display
            FROM records 
...
def get_leaderboard_optimized(fish: str = None, waterbody: str = None, limit: int = 100):
