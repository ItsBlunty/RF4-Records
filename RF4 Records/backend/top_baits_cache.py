...
def generate_top_baits_cache():
    ...
    try:
        ...
        # Generate the data
        data = get_top_baits_data_optimized()
        ...
        logger.info(
            f"✅ Top baits cache generated in {metadata['generation_time']:.3f}s"
        )
        logger.info(
            f"   Records: {metadata['total_records']}, Fish: {metadata['total_fish_species']}"
        )
