...
from simplified_records import (
...
def main():
    ...
    try:
        ...
        if total_records == 0:
            ...
            print("\n🔧 TO FIX:")
            print("   - If this is local development: Import data or connect to production DB")
        ...
        for period_name, (start_date, end_date) in periods.items():
            ...
            if count == 0:
                print(f"  ⚠️  NO RECORDS FOUND IN THIS PERIOD! This is likely the issue.")
            elif count < 10:
        ...
        for period_name, (start_date, end_date) in periods.items():
            ...
            if not samples:
                ...
                for i, record in enumerate(samples, 1):
                    print(f"    {i}. ID:{record['id']} | {record['player']} | {record['fish']} | "
        ...
        for check_name, base_date, hours_offset in boundary_checks:
            ...
            count = db.query(Record).filter(
                Record.created_at >= check_start,
                Record.created_at <= check_end
            ).count()
            
            print(f"{check_name}: {count} records")
            print(f"  Time range: {format_datetime(check_start)} to {format_datetime(check_end)}")
        ...
        recent_records = db.query(Record).order_by(Record.created_at.desc()).limit(5).all()
        ...
        if newest_record:
            ...
            if hours_since_newest > 24:
                print(f"⚠️  Data freshness issue: Newest record is {hours_since_newest:.1f} hours old")
                print("   - This might explain why recent periods have no data")
            else:
                print(f"✅ Data is fresh: Newest record is {hours_since_newest:.1f} hours old")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ['-h', '--help', 'help']:
        ...
        print("=============================")
        print("\nUsage:")
        print("  python debug_top_baits_dates.py              # Debug with current database")
